...
def test_唯一一对多组合只有相同摘要时仍须列为疑点():
    bank = _std_df([("2026-01-02", 60000, "项目回款", 1)])
    journal = _std_df(
        [
            ("2026-01-02", 24000, "项目回款", 10),
            ("2026-01-03", 36000, "项目回款", 11),
        ]
    )
...
def test_无业务边界的多解总额闭合也不能输出整组勾稽一致():
    bank = _std_df(
        [
            ("2026-01-02", 60000, "项目回款", 1),
            ("2026-01-02", 60000, "项目回款", 2),
        ]
    )
    journal = _std_df(
        [
            ("2026-01-02", 24000, "项目回款", 10),
            ("2026-01-02", 36000, "项目回款", 11),
            ("2026-01-02", 18000, "项目回款", 12),
            ("2026-01-02", 42000, "项目回款", 13),
        ]
    )
    matcher = Matcher(bank, journal, ds.MatcherConfig(dfs_date_window=3))
