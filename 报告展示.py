...
阅读表 = ('核对结论', '非工作日交易', '月度核对', '逐笔核对', '整组核对', '人工全查', '人工抽样', '未对应记录')
...
def format_display_sheets(book):
    ...
    hs = _headers(summary)
    messages = {
        '使用顺序': '先看非工作日银行收付示警，再看月度收支与余额、逐笔或整组。整组左侧“＋”展开原始记录。人工全查、人工抽样直接在黄色格选择结果。',
        '金额差额口径': '收入与支出分别比较。整组汇总金额写在文字格；收入、支出数值列只记录原始组成，避免重复加总。逐笔金额正数为收入、负数为支出。',
        '月度说明': '月度收支和余额在同一张表。人工确认不改变原始金额、记账月份和客观差额；年度合计只加总发生额。',
        '专题说明': '非工作日交易按银行所选日期独立示警；每日、退款重付、手续费、跨期、重复线索及输入详情保留在隐藏表中。',
...
def _style(sheet):
    ...
    widths = {'核对编号': 9, '行别': 12, '银行日期': 12, '序时账日期': 12, '日期': 12, '交易日期': 14, '原日期列名': 18, '原日期值': 18, '原文件行号': 14, '休息日类型': 16, '节日名称': 18, '收支方向': 12, '对方及辅助文字': 36, '银行记录': 34, '序时账记录': 34, '银行金额': 14, '序时账金额': 14, '金额': 14, '差额': 13, '核对差额': 22, '人工核对结果': 24, '当前核对结论': 29, '当前记录结果': 27, '当前核对结果': 30, '核对原因': 36, '核对依据': 36, '核对方式': 12, '摘要': 30, '对方及凭证': 27, '原始出处': 28, '银行原始出处': 26, '序时账原始出处': 26, '备注': 25}
