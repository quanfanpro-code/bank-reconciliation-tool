...
class Reporter:
    ...
    def _generate_report_legacy(
        ...
        date_format : str
            日期格式，默认 "auto"。
        """
        bank, journal = self.matcher.bank, self.matcher.journal
        run_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        # === 期初余额验证 ===
        journal_source = self.raw_journal if self.raw_journal is not None else journal
        journal_initial = BalanceRecalculator.extract_initial_balance(journal_source, self.journal_mapping)
        bank_initial = BalanceRecalculator.extract_initial_balance(bank, self.bank_mapping)
        initial_diff = abs(bank_initial - journal_initial)

        # 任一侧没有有效余额列数据时，期初核对无意义，跳过而非误报
        journal_has_balance = self._has_balance_data(journal_source, self.journal_mapping)
        bank_has_balance = self._has_balance_data(bank, self.bank_mapping)
        balance_check_possible = bank_has_balance and journal_has_balance
        has_warning = balance_check_possible and initial_diff > Decimal('0.01')
        self.initial_balance_warning = InitialBalanceWarning(
            has_warning=has_warning,
            bank_initial=bank_initial,
            journal_initial=journal_initial,
            diff=initial_diff,
            message=f"⚠️ 期初余额不一致！银行期初: {bank_initial:,.2f}, 日记账期初: {journal_initial:,.2f}, 差额: {initial_diff:,.2f}" if has_warning else ""
        )

        if has_warning and self.logger:
            self.logger("=" * 60)
            self.logger("⚠️【重要警告】期初余额不一致！")
            self.logger(f"   银行期初余额: {bank_initial:,.2f}")
            self.logger(f"   日记账期初余额: {journal_initial:,.2f}")
            self.logger(f"   差额: {initial_diff:,.2f}")
            self.logger("   请核对后再进行流水核对！")
            self.logger("=" * 60)
        ...
        if has_warning:
            summary.extend([
                ('⚠️⚠️⚠️ 期初余额差异警告 ⚠️⚠️⚠️', ''),
                ('银行期初余额', float(bank_initial)),
                ('日记账期初余额', float(journal_initial)),
                ('期初差额', float(initial_diff)),
                ('警告', '请先核对期初余额！'),
                ('', ''),
            ])

        def add_stats(df, name):
            inc, exp = df[df['amount'] > 0], df[df['amount'] < 0]
            summary.extend([
                (f'{name}总笔数', len(df)), (f'{name}总金额', round_decimal(df['amount'].sum())),
                (f'{name}收入笔数', len(inc)), (f'{name}收入金额', round_decimal(inc['amount'].sum())),
                (f'{name}支出笔数', len(exp)), (f'{name}支出金额', round_decimal(exp['amount'].sum())),
                ('', '')
            ])

        add_stats(bank, '银行流水')
        add_stats(journal, '日记账')

        match_ratio_bank = bank['matched'].sum() / len(bank) if len(bank) > 0 else 0
        match_ratio_journal = journal['matched'].sum() / len(journal) if len(journal) > 0 else 0

        matched_bank = bank[bank['matched']]
        matched_journal = journal[journal['matched']]
        low_conf_bank = len(matched_bank[matched_bank['confidence'] == '低'])
        total_matches = len(set(matched_bank['match_id']) | set(matched_journal['match_id']))
        low_conf_ratio = low_conf_bank / total_matches if total_matches > 0 else 0

        summary.extend([
            ('银行已匹配笔数', bank['matched'].sum()),
            ('日记账已匹配笔数', journal['matched'].sum()),
            ('银行匹配率', f"{match_ratio_bank:.1%}"),
            ('日记账匹配率', f"{match_ratio_journal:.1%}"),
            ('', '')
        ])
        ...
        if low_conf_bank > 0:
            summary.extend([
                ('=== ⚠️ 低置信度匹配统计 ===', ''),
                ('低置信度匹配笔数', low_conf_bank),
                ('低置信度占比', f"{low_conf_ratio:.1%}"),
                ('提示', '请重点人工复核！'),
                ('', '')
            ])

        if hasattr(self.matcher, 'exception_logger'):
            exc_summary = self.matcher.exception_logger.get_summary()
            if exc_summary['total_exceptions'] > 0:
                summary.extend([
                    ('=== 运行异常统计 ===', ''),
                    ('运行异常总数', exc_summary['total_exceptions']),
                ])
                for stage, count in exc_summary['counts_by_stage'].items():
                    summary.append((f'  - {stage}阶段异常', count))
                for sample in exc_summary['samples'][:3]:
                    summary.append((f"  样例[{sample['stage']}]", f"{sample['exception_type']}: {sample['message'][:50]}"))
                summary.append(('', ''))

        add_stats(bank[~bank['matched']], '银行未匹配')
        add_stats(journal[~journal['matched']], '日记账未匹配')
        ...
        if balance_check_possible:
            summary.extend([
                ('银行期初余额', float(bank_initial)),
                ('日记账期初余额', float(journal_initial)),
                ('期初余额差额', float(initial_diff)),
                ('期初余额状态', '✅ 一致' if not has_warning else '❌ 不一致 - 请检查！'),
            ])
        else:
            summary.append(('期初余额核对', '任一方未提供有效余额列数据，跳过期初核对'))
        df_sum = pd.DataFrame(summary, columns=['项目', '数值'])
        # 数值项保留数字类型（int/float），Excel 中可直接计算

        # === 2. 每日 / 月度统计 ===
        df_daily = self.calculate_daily_stats(bank, journal)
        if df_daily.empty:
            # 数据为空时仅输出汇总
            make_excel([('核对汇总', df_sum)], output_path, theme='deep-navy')
            return

        # 重算余额替换源文件余额
        bank_recalc = BalanceRecalculator()
        bank_daily_balances = bank_recalc.recalculate(bank)
        if bank_daily_balances:
            bank_bal_df = pd.DataFrame([
                {'date': b.date, 'balance_bank_recalc': b.balance}
                for b in bank_daily_balances
            ])
            df_daily = pd.merge(df_daily, bank_bal_df, on='date', how='left')
            if 'balance_bank_recalc' in df_daily.columns:
                df_daily['balance_bank'] = df_daily['balance_bank_recalc'].ffill()
                df_daily = df_daily.drop(columns=['balance_bank_recalc'])

        journal_recalc = BalanceRecalculator()
        journal_daily_balances = journal_recalc.recalculate(journal)
        if journal_daily_balances:
            journal_bal_df = pd.DataFrame([
                {'date': b.date, 'balance_journal_recalc': b.balance}
                for b in journal_daily_balances
            ])
            df_daily = pd.merge(df_daily, journal_bal_df, on='date', how='left')
            if 'balance_journal_recalc' in df_daily.columns:
                df_daily['balance_journal'] = df_daily['balance_journal_recalc'].ffill()
                df_daily = df_daily.drop(columns=['balance_journal_recalc'])

        # 重新计算余额差异
        if 'balance_bank' in df_daily.columns and 'balance_journal' in df_daily.columns:
            df_daily['balance_diff'] = (df_daily['balance_bank'] - df_daily['balance_journal']).apply(round_decimal)

        df_daily_ordered = df_daily[self._get_ordered_columns(df_daily)]
        df_daily_cn = df_daily_ordered.rename(columns={k: v for k, v in self.RENAME_MAP_DAILY.items() if k in df_daily_ordered.columns})

        # 确保值都是字符串或数字
        for col in df_daily_cn.columns:
            if df_daily_cn[col].dtype == 'object':
                df_daily_cn[col] = df_daily_cn[col].apply(lambda x: str(x) if pd.notna(x) else '')

        # 月度统计
        df_monthly = self.calculate_monthly_stats(df_daily)
        df_monthly_cn = df_monthly.rename(columns={k: v for k, v in self.RENAME_MAP_MONTHLY.items() if k in df_monthly.columns})

        for col in df_monthly_cn.columns:
            if df_monthly_cn[col].dtype == 'object':
                df_monthly_cn[col] = df_monthly_cn[col].apply(lambda x: str(x) if pd.notna(x) else '')

        # === 3. 匹配明细 ===
        details = []
        all_ids = {mid for mid in
                   (set(bank[bank['matched']]['match_id']) | set(journal[journal['matched']]['match_id']))
                   if mid}
        type_map = {
            'exact_1to1': '精确', 'monthly_total': '月核销', 'daily_total': '日核销',
            'combination_dfs': '组合', 'tolerance_date': '容差', 'batch_aggregation': '聚合'
        }

        match_evidence = {}
        for m in self.matcher.matches:
            match_evidence[m['id']] = {
                'match_stage': m.get('match_stage', ''),
                'amount_diff': m.get('amount_diff', Decimal('0')),
                'date_diff_days': m.get('date_diff_days', 0),
                'summary_similarity': m.get('summary_similarity', 0.0),
                'combo_count': m.get('combo_count', 1),
                'is_rule_matched': m.get('is_rule_matched', False),
                'is_tolerance_matched': m.get('is_tolerance_matched', False),
                'is_aggregation_matched': m.get('is_aggregation_matched', False)
            }

        def _match_order_key(mid):
            """确定性排序键：最早日期 -> 最大金额 -> match_id，保证报告可复现。"""
            b_grp = bank[bank['match_id'] == mid]
            j_grp = journal[journal['match_id'] == mid]
            dates = [d for d in list(b_grp['date']) + list(j_grp['date']) if pd.notna(d)]
            min_date = min(dates) if dates else pd.Timestamp.max
            amounts = b_grp['amount'].tolist() + j_grp['amount'].tolist()
            max_amt = max((abs(a) for a in amounts), default=Decimal('0'))
            return (min_date, float(max_amt), str(mid))

        for mid in sorted(all_ids, key=_match_order_key):
            b = bank[bank['match_id'] == mid]
            j = journal[journal['match_id'] == mid]
            if b.empty and j.empty:
                continue

            mtype = b.iloc[0]['match_type'] if not b.empty else j.iloc[0]['match_type']
            conf = b.iloc[0]['confidence'] if not b.empty else j.iloc[0]['confidence']
            diff = abs(b['amount'].sum() - j['amount'].sum())
            is_bad = diff > Decimal('0.01')
            is_low_conf = conf == '低'

            evidence = match_evidence.get(mid, {})
            match_stage = evidence.get('match_stage', '')
            date_diff_days = evidence.get('date_diff_days', 0)
            summary_similarity = evidence.get('summary_similarity', 0.0)
            combo_count = evidence.get('combo_count', 1)
            is_rule_matched = evidence.get('is_rule_matched', False)
            is_tolerance_matched = evidence.get('is_tolerance_matched', False)
            is_aggregation_matched = evidence.get('is_aggregation_matched', False)

            evidence_str = ''
            if is_rule_matched:
                evidence_str = '规则'
            elif is_tolerance_matched:
                evidence_str = '容差'
            elif is_aggregation_matched:
                evidence_str = '聚合'
            elif combo_count > 1:
                evidence_str = f'{combo_count}笔'

            for i in range(max(len(b), len(j))):
                row = {
                    '匹配ID': mid[:8],
                    '阶段': match_stage,
                    '类型': type_map.get(mtype, mtype),
                    '置信度': '⚠️低置信度' if is_low_conf else conf,
                    '金额差': float(diff) if diff != Decimal('0') else 0,
                    '日期差': date_diff_days,
                    '相似度': f'{summary_similarity:.2f}' if summary_similarity > 0 else '',
                    '笔数': combo_count if combo_count > 1 else '',
                    '证据': evidence_str,
                    '疑': '⚠️' if is_bad else '',
                    '_低置信度标记': is_low_conf
                }
                if i < len(b):
                    bank_row_number = b.iloc[i].get('original_file_row', b.iloc[i]['original_idx'])
                    row.update({
                        '银_日期': b.iloc[i]['date'], '银_金额': b.iloc[i]['amount'],
                        '银_摘要': b.iloc[i]['summary'], '银_行': bank_row_number
                    })
                if i < len(j):
                    r = j.iloc[i]
                    v = ''
                    if self.raw_journal is not None:
                        idx = int(r['original_idx']) - 1
                        if 0 <= idx < len(self.raw_journal):
                            vc = self.journal_mapping.get('voucher')
                            if vc and vc in self.raw_journal.columns:
                                v = clean_excel_string(self.raw_journal.iloc[idx][vc])
                    row.update({
                        '账_日期': r['date'], '账_金额': r['amount'],
                        '账_摘要': r['summary'], '账_凭证': v
                    })
                details.append(row)
        df_details = pd.DataFrame(details)

        # === 4. 未匹配记录 ===
        def get_unmatched(df, raw):
            if df.empty:
                return pd.DataFrame()
            if raw is not None:
                idxs = [int(i) - 1 for i in df['original_idx'] if 0 <= int(i) - 1 < len(raw)]
                res = raw.iloc[idxs].copy()
                for col in res.columns:
                    if res[col].dtype == object:
                        res[col] = res[col].apply(clean_excel_string)
                # 金额/余额类列经字符串清洗后还原为数值，保证 Excel 中可直接求和
                for col in res.columns:
                    col_text = str(col).lower()
                    if any(k in col_text for k in ('金额', 'amount', '发生额', '余额', 'balance')):
                        res[col] = res[col].apply(_restore_numeric_cells)
                amount_col = None
                for col in res.columns:
                    col_lower = str(col).lower()
                    if '金额' in col_lower or 'amount' in col_lower or '发生额' in col_lower:
                        amount_col = col
                        break
                if amount_col:
                    res['_sort_key'] = res[amount_col].apply(
                        lambda x: abs(float(x)) if pd.notna(x) and str(x).replace('-', '').replace('.', '').isdigit() else 0
                    )
                    res = res.sort_values(by='_sort_key', ascending=False).drop(columns=['_sort_key'])
                return res
            res = df[['date', 'amount', 'summary']].copy()
            res = res.sort_values(by='amount', key=lambda x: abs(x), ascending=False)
            return res

        df_un_b = get_unmatched(bank[~bank['matched']], self.raw_bank)
        df_un_j = get_unmatched(journal[~journal['matched']], self.raw_journal)

        # === 5. 余额差异核对 ===
        reconciler = BalanceReconciler(
            bank_balances=bank_daily_balances,
            journal_balances=journal_daily_balances
        )
        diffs = reconciler.generate_diff_report()

        df_balance_diff = pd.DataFrame()
        if diffs:
            diff_rows = []
            for d in diffs:
                bank_bal = d.bank_balance if d.bank_balance is not None else Decimal('0')
                journal_bal = d.journal_balance if d.journal_balance is not None else Decimal('0')
                diff_val = d.diff if d.diff is not None else Decimal('0')
                diff_rows.append({
                    '日期': d.date.strftime('%Y-%m-%d'),
                    '银行余额': round_decimal(bank_bal),
                    '日记账余额': round_decimal(journal_bal),
                    '差额': round_decimal(diff_val),
                    '差异类型': d.diff_type
                })
            df_balance_diff = pd.DataFrame(diff_rows)

        # === 6. 余额连续性异常（仅对有余额数据的一侧检查）===
        continuity_rows = []
        if bank_has_balance:
            continuity_rows.extend(self.check_balance_continuity(bank, source='银行流水'))
        if journal_has_balance:
            continuity_rows.extend(self.check_balance_continuity(journal, source='日记账'))
        df_continuity = pd.DataFrame(continuity_rows) if continuity_rows else pd.DataFrame()

        # ================================================================
        # 用 make_excel deep-navy 主题一次性生成
        # ================================================================
        sheets = []
        sheets.append(('核对汇总', df_sum))
        sheets.append(('每日统计', df_daily_cn))
        sheets.append(('月度统计', df_monthly_cn))
        if not df_details.empty:
            sheets.append(('匹配明细', df_details))
        sheets.append(('银行未达', df_un_b))
        sheets.append(('日记账未达', df_un_j))
        if not df_balance_diff.empty:
            sheets.append(('余额差异明细', df_balance_diff))
        if not df_continuity.empty:
            sheets.append(('余额连续性异常', df_continuity))

        make_excel(sheets, output_path, theme='deep-navy')

        # ================================================================
        # openpyxl 后处理：条件格式 + 附加 sheet
        # ================================================================
        wb = load_workbook(output_path)

        # --- 汇总表：期初警告红字/黄底 ---
        has_initial_warning = self.initial_balance_warning and self.initial_balance_warning.has_warning
        if '核对汇总' in wb.sheetnames:
            self._postprocess_summary(wb['核对汇总'], has_initial_warning)

        # --- 匹配明细：低置信度标记 + 差额条件格式 ---
        if '匹配明细' in wb.sheetnames:
            self._postprocess_details(wb['匹配明细'])

        # --- 每日/月度/余额差异/连续性：差额列条件格式 ---
        for sheet_name in ['每日统计', '月度统计', '余额差异明细', '余额连续性异常']:
            if sheet_name in wb.sheetnames:
                self._postprocess_diff_columns(wb[sheet_name])

        # === 运行参数表 ===
        if config is not None:
            params_data = [
                ('参数名称', '参数值'),
                ('容差天数', config.tolerance_days),
                ('组合窗口(天)', config.dfs_date_window),
                ('最大深度', config.max_dfs_depth),
                ('是否允许异号', '是' if config.allow_mixed_sign else '否'),
                ('贪心次数', config.greedy_attempts),
                ('日期格式', date_format),
                ('银行文件路径', bank_path or ''),
                ('日记账文件路径', journal_path or ''),
                ('随机种子', config.random_seed),
                ('运行时间戳', run_timestamp),
            ]
            ws_params = wb.create_sheet('运行参数')
            header_font_white = Font(bold=True, color='FFFFFF')
            header_fill = PatternFill(start_color='1F4E79', end_color='1F4E79', fill_type='solid')
            for r_idx, (param_name, param_val) in enumerate(params_data, 1):
                ws_params.cell(row=r_idx, column=1, value=param_name)
                ws_params.cell(row=r_idx, column=2, value=param_val)
                if r_idx == 1:
                    for c in range(1, 3):
                        ws_params.cell(row=r_idx, column=c).font = header_font_white
                        ws_params.cell(row=r_idx, column=c).fill = header_fill
        ...
        if self.error_collector and self.error_collector.has_errors():
            parse_errors = self.error_collector.get_all_errors()
            if parse_errors:
                df_parse_errors = pd.DataFrame(parse_errors)
                ws_errors = wb.create_sheet('解析异常明细')
                error_header_font = Font(bold=True, color='FFFFFF')
                error_header_fill = PatternFill(start_color='FF6B6B', end_color='FF6B6B', fill_type='solid')

                for r_idx, row in enumerate(dataframe_to_rows(df_parse_errors, index=False, header=True), 1):
                    for c_idx, value in enumerate(row, 1):
                        cell = ws_errors.cell(row=r_idx, column=c_idx)
                        if pd.notna(value):
